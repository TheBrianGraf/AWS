...
<# EXAMPLE:
$vcenter = 'vcenter.sddc-55-32-57-20.vmc.vmware.com'
$vcuser =  'brian@VMware.local'
$vcpass =  'VMware1!'
...
$vcuser =  ''
$vcpass =  ''
$sqsqueuURL = 'https://sqs.us-west-2.amazonaws.com/092437863332/test'
...
while ($sw.elapsed -lt $timeout) {
    ...
    $Button = Receive-SQSMessage -QueueUrl "$sqsqueuURL" # Add the Queue you create in AWS here. example: "https://sqs.us-west-2.amazonaws.com/0924373867744/test"
    ...
    if ($Button -ne $Null){
        ...
        switch ($type) {
            ...
            "Single" {
                ...
                # Either add a resource pool with the name IOT-DEMO, or rename the resource pool
                if(
				    ($rp = Get-ResourcePool "IOT-DEMO" -server $connection1).count -gt 1
				) {
                    write-host "More than one RP"
                    $rp = Get-ResourcePool "IOT-DEMO" -server $connection1 | select-object -first 1  #ADD Your resource pool where you want the VMs to be deployed to
                    }
                $ds = get-datastore "WorkloadDatastore" -server $connection1
                $tmpl = get-template "photon" -server $connection1 # Change to a template you want to use
         ...
         Remove-SQSMessage -QueueUrl "$sqsqueuURL" -ReceiptHandle $button.receipthandle -Force
    }
